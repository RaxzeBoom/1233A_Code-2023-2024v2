...
{
  //Canges the drive motors to brake to help with auto
  Change_DT_Brake(2);
  Reset_PID_Drive();
  //defines variables to be used later in the PID loop
  double Combine_Ticks = 0;
  bool Exit = false;
  double Ac_Error = 0;
  double Prev_Error = (Ls_SIDE_RS.get_position() + Rs_SIDE_RS.get_position())/2;
  //Sets how much the motors need to more
  ...
  while(Exit == false)
  {
    ...
    Ac_Error += Error;
    double power =  Striaght_PID[0] * Error + Striaght_PID[1] * Ac_Error + Striaght_PID[2]*(Error-Prev_Error);
    //The PID loop using numbers that are set in the config file
    double Diff_power = Striaght_PID[3] * (R_Side_Ticks-L_Side_Ticks);
    //The P loop that help with adjusting the speed the both side the drivetrain to match with each other
    power = std::fmin(power , maxspeed - 5);
    //makes sure that power is below maxspeed with some room for the P loop
    if(Target_Dis > 0)
    {
    Ls_speed = power +(Diff_power/2);
    Rs_speed = power -(Diff_power/2);
    }
    ...
    { }//Does nothing if Target Distance is not negitive or postive
    Ls_speed = std::fmin(Ls_speed , maxspeed);
    Rs_speed = std::fmin(Rs_speed , maxspeed);
    //Makes sure that the speed never goes above maxspeed
    ...
    //the exit condition to exit the loop when the wheels are where they need to be
    Prev_Error = Error;
  }
}
void PID_TURN(double Target_Degree, double maxspeed)
{
  Change_DT_Brake(2);
  Reset_PID_Drive();
  double Ac_Error = 0;
  const double Target_Ticks = (((Target_Degree/360)*Base_Width)/Wheel_Size)*36000;
  double Prev_Error = 0;
  int Ls_speed = 0;
  int Rs_speed = 0;
  while(true)
  {
    double L_Side_Ticks = Ls_SIDE_RS.get_position();
    double R_Side_Ticks = Rs_SIDE_RS.get_position();
    double B_Side_Ticks = (fabs(R_Side_Ticks)+fabs(L_Side_Ticks))/2;
    double Error = Target_Ticks - B_Side_Ticks;
    Ac_Error += Error;
    int power =  Turn_PID[0] * Error + Turn_PID[1] * Ac_Error + Turn_PID[2]*(Error-Prev_Error);
    if(power > maxspeed-5)
    {
      power = maxspeed-5;
    }
    int Diff_power = Turn_PID[3] * (fabs(R_Side_Ticks)-fabs(L_Side_Ticks));
    if(Target_Degree > 0)
    {
    Ls_speed = power -(Diff_power/2);
    Rs_speed = power -(Diff_power/2);
    ...
    if(Rs_speed > maxspeed)
    {
      ...
      SetLeftDTPower(Ls_speed);
      SetRightDTPower(Rs_speed);
if((((B_Side_Ticks <= Target_Ticks + 15)&(B_Side_Ticks >= Target_Ticks - 15)) & ((fabs(L_Side_Ticks) <= fabs(R_Side_Ticks) + 20) & (fabs(L_Side_Ticks) >= fabs(R_Side_Ticks) - 20)))     )
    {
      break;
    }
    Prev_Error = Error;
...
void PID_CURVE(double Target_Degree, double Radius , double maxspeed)
{
  Change_DT_Brake(2);
  Reset_PID_Drive();
  bool Exit = false;
  ...
  int TNEG = 1;
  int RNEG = 1;
  if(Target_Degree < 0) TNEG = -1;
  if(Radius < 0) RNEG = -1;
  const double Wheel_Radius = Base_Width * TNEG * RNEG;
  const double Ls_Target_Ticks = (((Target_Degree/360)*(Radius + Wheel_Radius))/Wheel_Size)*36000;
  const double Rs_Target_Ticks = (((Target_Degree/360)*(Radius - Wheel_Radius))/Wheel_Size)*36000;
  double Ls_Prev_Error = 0, Rs_Prev_Error = 0;
  int Ls_speed = 0, Rs_speed = 0;
  while(Exit == false)
  {
    double L_Side_Ticks = Ls_SIDE_RS.get_position();
    double R_Side_Ticks = Rs_SIDE_RS.get_position();
    ...
    int Ls_power =  Turn_PID[0] * Ls_Error + Turn_PID[1] * Ls_Ac_Error + Turn_PID[2]*(Ls_Error-Ls_Prev_Error);
    int Rs_power =  Turn_PID[0] * Rs_Error + Turn_PID[1] * Rs_Ac_Error + Turn_PID[2]*(Rs_Error-Rs_Prev_Error);
    int Diff_power = Turn_PID[3] * (fabs(R_Side_Ticks)-fabs(L_Side_Ticks));
    if(Target_Degree > 0)
    {
    Ls_speed = std::min(Ls_power, maxspeed-5) -(Diff_power/2);
    Rs_speed = power -(Diff_power/2);
    }
    else if(Target_Degree < 0)
    {
...
void AutoDrive(double inches, double maxPct) {
  ...
  double shortestAngle = fmod((angle-0+540.0), 360.0)-180.0;
  ...
  while (pros::millis() < escapeTime + 60) { // Exit the loop if the angle is within the margin of error and the speed is below 5 (Speed cutoff prevents overshoot)
    ...
    shortestAngle = fmod((angle-0+540.0), 360.0)-180.0;
